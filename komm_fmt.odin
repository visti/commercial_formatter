...
process_line :: proc(file: ^string) {
    processed_lines := ""

    for line in str.split_lines_iterator(file) {
        modified_line := line
        for _ ,pos in bauerPos {
            //fmt.printf("%v \n", line[:bauerPos[pos]])
            a := [?]string {modified_line[:bauerPos[pos]], ";", modified_line[bauerPos[pos]:]}
